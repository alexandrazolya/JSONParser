...
void test::removeKeyChildDuplicates(std::map <std::string, std::vector<JSONNode*>> &kC1, std::map <std::string, std::vector<JSONNode*>> &kC2)
{ //NOT USED
	...
	else { std::cout << "Different sizes of keyChild" << std::endl; }
}
...
bool test::operator==(const JSONNode &node1, const JSONNode &node2)
{
	...
	if (!(node1.keyChild == node2.keyChild)) { return false; }
...
{
	if (node1->keyChild.size() == node2->keyChild.size())
	{
		for (auto it = node1->keyChild.begin(); it != node1->keyChild.end(); ++it)
		{
			for (auto childIT = it->second.begin(); childIT != it->second.end(); ++childIT)
			{
				size_t index = std::distance(it->second.begin(), childIT);
				for (auto& pair : node2->keyChild)
					compare(*childIT, *(pair.second.begin() + index));
			}
		}
		if (node1->key == node2->key)
		{
			if (node1->keyChild == node2->keyChild)
			{
				return true;
			}
			else
...
std::ostream& test::operator<<(std::ostream& os, const std::map<std::string, std::vector<JSONNode*>> &kC)
{
	...
	{
		os << p.first << ": ";
		for (JSONNode *n : p.second) os << n->keyChild << ' ';
		os << std::endl;
	}
	return os;
}
